...
export default function AdminReportsPage() {
  ...
  const downloadExcel = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `hys-rapor-${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (e: any) {
      alert("Excel indirilemedi: " + (e?.message ?? "bilinmeyen hata"));
    } finally {
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-xl border border-emerald-700/60 bg-emerald-900/40 px-4 py-2 text-sm font-semibold text-emerald-300 hover:bg-emerald-800/50 transition disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {excelLoading ? "Hazırlanıyor..." : "Excel İndir (.xlsx)"}
        </button>
      </div>
