  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-900/80 shadow-2xl shadow-slate-900/30 backdrop-blur">
        <div className="grid grid-cols-[2fr,1fr,1fr,1.5fr] px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400 border-b border-slate-800">
          <div>Mağaza</div>
          <div>Durum</div>
          <div>Öncelik</div>
          <div>Oluşturma</div>
        ...
        <div className="divide-y divide-slate-800/70">
          {tickets.map((t, idx) => {
            ...
            return (
              <Link
                ...
                className={`grid grid-cols-[2fr,1fr,1fr,1.5fr] px-4 py-3 text-sm transition hover:bg-slate-800/60 ${
                  idx % 2 === 0 ? "bg-slate-900/40" : "bg-slate-900/20"
                }`}
              >
                <span className="text-slate-100">
                  {store ? `${store.name} (${store.id})` : t.store_id}
                </span>
          ...
          {tickets.length === 0 && <div className="px-4 py-6 text-sm text-slate-400">Kayıt bulunamadı.</div>}
        </div>
      </div>
    </div>
  );
